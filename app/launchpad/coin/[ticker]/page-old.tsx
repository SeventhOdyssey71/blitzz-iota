...
import { useWalletKit } from "@iota/wallet-kit"
import { useBalance } from "@/hooks/use-balance"
import { toast } from "sonner"
import { BondingCurveInfo } from "@/lib/contracts/meme-token-factory"
...
export default function CoinDetailPage({ params }: { params: { ticker: string } }) {
  ...
  // Get user balances
  const { balance: iotaBalance } = useBalance("0x2::iota::IOTA")
  const { balance: tokenBalance } = useBalance(`${PACKAGE_ID}::${params.ticker}::${params.ticker.toUpperCase()}`)
  
  // Fetch bonding curve info
  useEffect(() => {
    const fetchInfo = async () => {
      if (MOCK_BONDING_CURVE_ID) {
        const info = await getBondingCurveInfo(MOCK_BONDING_CURVE_ID)
  ...
  const handleBuy = async () => {
    ...
    const iotaAmount = parseTokenAmount(buyAmount)
    await buyTokens(
      `${PACKAGE_ID}::${params.ticker}::${params.ticker.toUpperCase()}`,
  ...
  const marketCap = bondingCurveInfo ? 
    (parseFloat(formatTokenAmount(bondingCurveInfo.totalSupply)) * parseFloat(formatTokenAmount(bondingCurveInfo.iotaReserve)) / parseFloat(formatTokenAmount(BigInt(bondingCurveInfo.totalSupply) - BigInt(bondingCurveInfo.tokensSold))) * iotaPrice) 
    : 0
  
  const price = bondingCurveInfo ?
    parseFloat(formatTokenAmount(bondingCurveInfo.iotaReserve)) / parseFloat(formatTokenAmount(BigInt(bondingCurveInfo.totalSupply) - BigInt(bondingCurveInfo.tokensSold)))
    : 0
              ...
              <CardContent className="p-8">
                <div className="flex items-center gap-6 mb-8">
                  <div className="relative">
                    <Image
                      src="/placeholder.svg"
                      alt={bondingCurveInfo?.name || "Token"}
                      width={80}
                      height={80}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                  <div className="text-center">
                    ...
                    <p className="text-white font-semibold text-lg font-mono tabular-nums">{coinData.age}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-gray-400 text-sm mb-1">Price USD</p>
                    <p className="text-white font-semibold text-lg font-mono tabular-nums">{coinData.priceUSD}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-gray-400 text-sm mb-1">Price IOTA</p>
                    <p className="text-white font-semibold text-lg font-mono tabular-nums">{coinData.priceIOTA}</p>
                  </div>
                  <div></div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-6 mb-8">
                  <div className="text-center p-4 bg-black/50 rounded-xl border border-white/10">
                    ...
                    <p className="text-white font-bold text-xl font-mono tabular-nums">{coinData.marketCap}</p>
                  </div>
                  <div className="text-center p-4 bg-black/50 rounded-xl border border-white/10">
                    <p className="text-gray-400 text-sm mb-2">24h Volume</p>
                    <p className="text-white font-bold text-xl font-mono tabular-nums">{coinData.volume24h}</p>
                  </div>
                  <div className="text-center p-4 bg-black/50 rounded-xl border border-white/10">
                    <p className="text-gray-400 text-sm mb-2">Liquidity</p>
                    <p className="text-white font-bold text-xl font-mono tabular-nums">{coinData.liquidity}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-cyan-400 font-mono">{coinData.createdBy}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-gray-400">Market Cap</span>
                    <span className="text-white font-semibold font-mono tabular-nums">{coinData.marketCap}</span>
                    <span className="text-green-400 font-mono tabular-nums">(+{coinData.change})</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-8">
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white font-medium">{coinData.ticker}/IOTA · 1 · Launchpad</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                <div className="h-96 bg-black/20 backdrop-blur-sm rounded-xl border border-white/10 flex items-center justify-center mb-8">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 text-sm">Real-time price data will be displayed here</p>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-gray-400 text-sm font-mono tabular-nums">Balance: 100.4238</span>
                      </div>
                    </div>
                    <div className="relative">
                      <Input
                        ...
                        className="bg-black/50 backdrop-blur-sm border-white/10 text-white text-center text-2xl font-bold h-16 rounded-xl focus:border-cyan-500/50"
                        placeholder="0.00"
                      />
                      <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
                        <div className="w-6 h-6 bg-cyan-500 rounded-full flex items-center justify-center">
                          <span className="text-black text-xs font-bold">IOTA</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Input
                        value="0"
                        readOnly
                        className="bg-black/50 backdrop-blur-sm border-white/10 text-white text-center text-2xl font-bold h-16 rounded-xl"
                      />
                      <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
                        <div className="w-6 h-6 bg-gray-600 rounded-full flex items-center justify-center">
                          <span className="text-white text-xs font-bold">{coinData.ticker}</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <Card className="glass-dark border border-white/10 rounded-2xl">
              <CardContent className="p-8">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-cyan-400 font-bold text-lg font-mono tabular-nums">{coinData.bondingCurveProgress}%</span>
                </div>
                ...
                <div className="space-y-4 text-sm leading-relaxed">
                  <div className="p-4 bg-black/50 rounded-xl border border-white/10">
                    <p className="text-white/80">
                      <span className="text-cyan-400 font-semibold">{coinData.availableTokens}</span>{" "}
                      <span className="text-white font-medium">${coinData.ticker}</span> tokens available for sale
                    </p>
                  </div>

                  <div className="p-4 bg-black/50 rounded-xl border border-white/10">
                    <p className="text-white/80">
                      <span className="text-cyan-400 font-semibold">{coinData.bondingCurveTokens}</span>{" "}
                  ...
                  <div className="p-4 bg-cyan-500/10 rounded-xl border border-cyan-500/20">
                    <p className="text-white/80">
                      When market cap reaches <span className="text-cyan-400 font-semibold">4,000 IOTA</span>, liquidity
