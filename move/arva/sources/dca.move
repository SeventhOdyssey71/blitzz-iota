module Blitz::dca {
    ...
    public entry fun create_dca_strategy<CoinA, CoinB>(
        ...
        assert!(interval_ms >= 60000, EInvalidInterval); // Minimum 1 minute interval
        assert!(total_orders > 0 && total_orders <= 365, EInvalidOrderCount); // Max 1 year daily DCA
    ...
    public entry fun execute_dca_order<CoinA, CoinB>(
        ...
        assert!(time_since_last >= strategy.interval_ms, EIntervalNotElapsed);
        ...
        if (strategy.min_amount_out > 0) {
            assert!(expected_output >= strategy.min_amount_out, EPriceOutOfRange);
        };
        if (strategy.max_amount_out > 0) {
            assert!(expected_output <= strategy.max_amount_out, EPriceOutOfRange);
        };
